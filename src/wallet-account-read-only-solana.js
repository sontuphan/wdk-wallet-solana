 ...
 * @property {string | string[]} [rpcUrl] - The provider's rpc url.
...
export default class WalletAccountReadOnlySolana extends WalletAccountReadOnly {
  ...
  constructor (addr, config = {}) {
    ...
    if (Array.isArray(rpcUrl)) {
      this._rpc = rpcUrl
        .reduce(
          /**
           * @param {FailoverProvider<SolanaRpc>} failover
           * @param {string} rpcUrl
           */
  ...
  async getBalance () {
    ...
    const balance = await this._rpc
      .getBalance(addr, { commitment: this._commitment })
      .send()
